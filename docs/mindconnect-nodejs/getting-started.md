...
You can use the command line mode to upload timeseries, files and create events in the {{site.productname}}.
...
Commands:
  ...
  upload-timeseries|ts [options]      parse .csv file with timeseriesdata and upload the timeseries data to {{site.productname}}
  upload-file|uf [options]            upload the file to the {{site.productname}} file service (optional: passkey) *
  create-event|ce [options]           create an event in the {{site.productname}} (optional: passkey) *
  agent-status|as [options]           displays the agent status and agent onboarding status *
  create-agent|ca [options]           create an agent in the {{site.productname}} *
  offboard-agent|of [options]         offboards the agent in the {{site.productname}} *
  ...
  download-bulk|db [options]          download the timeseries from {{site.productname}}
  list-assets|la [options]            list assets in the tenant *
  delete-asset|da [options]           delete asset with id <assetid> from {{site.productname}} *
  list-files|ls [options]             list files stored with the asset *
  download-file|df [options]          download the file from {{site.productname}} file service *
  delete-file|de [options]            delete the file from {{site.productname}} file service *
  identity-management|iam [options]   manage {{site.productname}} users and groups *
  ...
  dev-proxy|px [options]              starts {{site.productname}} development proxy & (optional passkey) *
  ...
  Documentation:
    ...
    the magenta colored commands * use app or service credentials or borrowed {{site.productname}} cookies
    the cyan colored commands require mindconnectlib (agent) credentials
    the blue colored commands @ use analytical functions of {{site.productname}}
    the green colored commands # are used as setup and utility commands
    the yellow colored commands & use borrowed {{site.productname}} application cookies
