...
Presented during {{site.productname}} Developer Days in Berlin, Sept. 2018:
...
- a simple todo app using the MEAN (MongoDB, Express.js, Angular, Node.js) stack
    ...
    - [<i class="fab fa-github"></i> Backend](https://github.com/mindsphere/devops-demo/tree/master/server)
        - the backend provides swagger-ui for navigating both the server apis and the {{site.productname}} APIs (under `/api-docs`)
- local Angular dev server setup that proxies requests to {{site.productname}}, allowing local development
- a devops admin backend that provides access to prometheus and grafana
  ...
  IoT data to the {{site.productname}} APIs
...
Please refer to the official {{site.productname}} & CloudFoundry developer documentation
...
- The gitlab-ci integration requires manually setting authentication with access and refresh tokens available as protected CI/CD Gitlab Variables, and they need to be renewed every 30 days. This can be copied directly from your CloudFoundry CLI ~/.cf/config.json file after successful cf login.
  Storage for Prometheus is currently transient, pending support for some kind of dynamic persistent storage for apps, or direct support for Prometheus in {{site.productname}}.
